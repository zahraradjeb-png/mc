...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openModal(id) {
    ...
    document.getElementById('rwCart')?.addEventListener('click', () => {
      if (!isLoggedIn()) { askLogin('ajouter des produits au panier'); return; }
      GoldAuth.addToCart({ id:String(p.id), name:p.name, price:p.price, emoji:'' });
      updateCartBadges();
  ...
  function bindCardEvents() {
    document.querySelectorAll('.add-cart-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        if (!isLoggedIn()) { askLogin('ajouter des produits au panier'); return; }
        GoldAuth.addToCart({ id:String(btn.dataset.id), name:btn.dataset.name, price:parseFloat(btn.dataset.price||0), emoji:'' });
        updateCartBadges();
        showToast(`"${btn.dataset.name}" ajouté au panier !`);
      });
    });
